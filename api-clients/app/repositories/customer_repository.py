...
class CustomerRepository(BaseRepository[Customer]):
    ...
    async def get_by_reference(self, reference: str) -> Optional[Customer]:
        ...
        result = await self.db.execute(select(Customer).where(Customer.reference == reference))
        return result.scalar_one_or_none()
    ...
    async def get_by_type(self, customer_type: CustomerType, skip: int = 0, limit: int = 100) -> List[Customer]:
        """Get customers by type."""
        result = await self.db.execute(select(Customer).where(Customer.type_client == customer_type).offset(skip).limit(limit))
        return list(result.scalars().all())

    async def get_by_status(self, status: CustomerStatus, skip: int = 0, limit: int = 100) -> List[Customer]:
        """Get customers by status."""
        result = await self.db.execute(select(Customer).where(Customer.statut == status).offset(skip).limit(limit))
        return list(result.scalars().all())

    async def count_by_type(self, customer_type: CustomerType) -> int:
        """Count customers by type."""
        result = await self.db.execute(select(func.count()).select_from(Customer).where(Customer.type_client == customer_type))
        return result.scalar_one()

    async def count_by_status(self, status: CustomerStatus) -> int:
        """Count customers by status."""
        result = await self.db.execute(select(func.count()).select_from(Customer).where(Customer.statut == status))
        return result.scalar_one()
